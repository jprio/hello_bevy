...
use bevy::{
    core_pipeline::{ bloom::{ BloomCompositeMode, BloomSettings }, tonemapping::Tonemapping },
    prelude::*,
...
fn setup(
    ...
    // Hexagon mesh
    commands.spawn(MaterialMesh2dBundle {
        mesh: meshes.add(shape::RegularPolygon::new(100, 6).into()).into(),
        // 4. Put something bright in a dark environment to see the effect
    ...
    match bloom_settings {
        (entity, Some(mut bloom_settings)) => {
            *text = "BloomSettings (Toggle: Space)\n".to_string();
            ...
            text.push_str(
                &format!("(W/S) Low-frequency boost: {}\n", bloom_settings.low_frequency_boost)
            );
            text.push_str(
                &format!(
            ...
            text.push_str(
                &format!("(R/F) High-pass frequency: {}\n", bloom_settings.high_pass_frequency)
            );
            text.push_str(
